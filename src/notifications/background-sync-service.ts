...
>() {
  static Default = Layer.scoped(
    BackgroundSyncService,
    Effect.gen(function* () {
      yield* Console.log('[BackgroundSyncService] Creating service');
...
export const ensureBackgroundSyncWorker = (get: Atom.Context) =>
  Effect.gen(function* () {
    if (workerStarted) {
      yield* Console.log('[BackgroundSync] Worker already running');
      return;
    }
    workerStarted = true;

    const service = yield* BackgroundSyncService;
    yield* Console.log('[BackgroundSync] Starting daemon worker');
    yield* Effect.forkDaemon(createBackgroundWorker(get, service.statusPubSub));
  });
